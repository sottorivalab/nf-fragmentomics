...
option_list <- list(
  make_option(
    ...
    default = 10000,
    help = paste(
      "Number of random points to generate in Monte Carlo integration",
      "[default %default]"
    ),
    metavar = "number",
    dest = "random.points"
...
if (sys.nframe() == 0L) {
  ...
  peak_data <- fragmentomics::peak_stats(
    ...
    left = opt$bg.limit.left,
    right = opt$bg.limit.right,
    central = opt$central.coverage.bp,
    average = 1000,
    rpoints = 10000
  )
...
# # READ MATRIX
# read_matrix <- function(mfile, opt) {
...
#   return(list(all=all.data, summary=summary.table, integration=integration.data, stats=peak.stats))
# }
...
# # PLOT SIGNAL
# plot_signal <- function(summary.table, integration.data, peak.stats, opt) {
#   # labels
