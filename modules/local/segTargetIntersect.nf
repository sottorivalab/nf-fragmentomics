process SEGTARGETINTERSECT {
    publishDir "${params.outdir}/${meta_sample.caseid}/${meta_sample.id}/fragmentomics/processed/matrix/${meta_target.source}/${meta_target.name}/ploidy", mode:'copy', overwrite:true
    ...
    output:
    tuple val(meta_sample), val(meta_target), path("*_GAIN.bed"), path("*_NEUT.bed"), emit: targets_ploidy
